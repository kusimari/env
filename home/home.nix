{ config, lib, pkgs, envKind, ... }:
{
  ...
  # SSH setup script - runs during activation to configure SSH
  home.file.".config/env/ssh-setup.sh" = {
    source = ./ssh-setup.sh;
    executable = true;
  };

  # Activation script - calls external SSH setup script
  # This runs on ALL systems during home-manager switch
  home.activation.setupSSHConfig = lib.hm.dag.entryAfter ["writeBoundary"] ''
    if [[ -f "$HOME/.config/env/ssh-setup.sh" ]]; then
      $DRY_RUN_CMD "$HOME/.config/env/ssh-setup.sh"
    fi
  '';
