...
export NIX_COMMAND='sudo -H nix run nix-darwin -- switch --flake "$FLAKE_DIR"'
export NIX_ECHO_MESSAGE="Running nix-darwin switch..."

# Option 2: Specific nix-darwin version (uncomment if needed)
# export NIX_COMMAND='sudo -H nix run nix-darwin/nix-darwin-25.05#darwin-rebuild -- switch --flake "$FLAKE_DIR"'
