#!/usr/bin/env bash
# env/layers/layer-1-ubuntu-mane.sh — Layer 1 (Host Genesis) for the
# ubuntu-mane envKind.
#
# Bare-metal prep: native packages (apt prereqs + hardware drivers)
# + the Determinate Nix installer (multi-user). Exits when done.
# No chaining; the user runs the next layer (layer-2.sh) separately.
#
# Curl-friendly:
...
log "Layer 1 (ubuntu-mane): Host Genesis$( (( DRY_RUN )) && echo ' (dry-run)')"
require_ubuntu
ensure_apt_prereqs
ensure_host_prep "$@"
ensure_nix
